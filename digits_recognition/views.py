...
import json
from digits_recognition import model
...
saver.restore(sess, "digits_recognition/data/regression.ckpt")
...
saver = tf.train.Saver(variables)
saver.restore(sess, "digits_recognition/data/convolutional.ckpt")
...
def process(request):
    #标准化数据
    input = ((255 - np.array(eval(request.POST.get('inputs')), dtype=np.float32)) / 255.0).reshape(1, 784)
    #一维数组，输出10个预测概率
